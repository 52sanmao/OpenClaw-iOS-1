"""GitHub API client for MCP discovery."""

import base64, time, sys
import requests

RATE_LIMIT_BUFFER = 50
RATE_LIMIT_BACKOFF = 10


class GitHubClient:
    def __init__(self, token: str):
        self.headers = {
            "Authorization": f"Bearer {token}",
            "Accept": "application/vnd.github+json",
            "X-GitHub-Api-Version": "2022-11-28",
        }
        self._remaining: int | None = None
        self._reset_at: float | None = None

    def _update_rate_limit(self, resp: requests.Response):
        remaining = resp.headers.get("X-RateLimit-Remaining")
        reset = resp.headers.get("X-RateLimit-Reset")
        if remaining is not None:
            self._remaining = int(remaining)
        if reset is not None:
            self._reset_at = float(reset)

    def _pace(self):
        if self._remaining is None:
            return
        if self._remaining <= 0 and self._reset_at:
            wait = max(0, self._reset_at - time.time()) + 1
            print(f"  rate limit exhausted, waiting {wait:.0f}s", file=sys.stderr, flush=True)
            time.sleep(wait)
        elif self._remaining < RATE_LIMIT_BUFFER:
            print(f"  rate limit low ({self._remaining} left), slowing down", file=sys.stderr, flush=True)
            time.sleep(RATE_LIMIT_BACKOFF)

    def search_repos(self, query: str, sort: str = "updated", order: str = "desc",
                     page: int = 1) -> list[dict]:
        self._pace()
        resp = requests.get(
            "https://api.github.com/search/repositories",
            headers=self.headers,
            params={"q": query, "sort": sort, "order": order, "per_page": 100, "page": page},
            timeout=15,
        )
        self._update_rate_limit(resp)
        if resp.status_code in (422, 403):
            return []
        resp.raise_for_status()
        return resp.json().get("items", [])

    def search_code(self, query: str) -> list[str]:
        """Code search — returns list of unique repo full_names."""
        self._pace()
        resp = requests.get(
            "https://api.github.com/search/code",
            headers=self.headers,
            params={"q": query, "per_page": 100},
            timeout=15,
        )
        self._update_rate_limit(resp)
        if resp.status_code in (422, 403):
            return []
        resp.raise_for_status()
        return list({item["repository"]["full_name"] for item in resp.json().get("items", [])})

    def fetch_repo(self, full_name: str) -> dict | None:
        self._pace()
        resp = requests.get(
            f"https://api.github.com/repos/{full_name}",
            headers=self.headers, timeout=15,
        )
        self._update_rate_limit(resp)
        return resp.json() if resp.status_code == 200 else None

    def fetch_file(self, owner: str, repo: str, path: str, branch: str = "main") -> str | None:
        refs = list(dict.fromkeys([branch, "main", "master"]))
        for ref in refs:
            self._pace()
            resp = requests.get(
                f"https://api.github.com/repos/{owner}/{repo}/contents/{path}?ref={ref}",
                headers=self.headers, timeout=15,
            )
            self._update_rate_limit(resp)
            if resp.status_code == 200:
                data = resp.json()
                if data.get("encoding") == "base64":
                    try:
                        return base64.b64decode(data["content"]).decode("utf-8", errors="replace")
                    except Exception:
                        return None
        return None

    @property
    def remaining(self) -> int | None:
        return self._remaining
